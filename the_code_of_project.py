...
lan = ttk.Combobox(t,value=["arabic"],width=27)
http://lan.place(x=5,y=0)
...
def translate_file(file,to="arabic"):
    ...
    return translation
def copy_translate() :
    with open(r"{}_1{}".format(inp_massar.get()[0:str(inp_massar.get()).index(".")],inp_massar.get()[str(inp_massar.get()).index("."):]),mode="r",encoding='utf-8') as text :
...
def translate_() :
    ...
    with open(r"{}".format(inp_massar.get()),mode="w",encoding='utf-8') as text :
        ...
        tts=gTTS(x,lang="ar")
        qw12=translate_file
        http://tts.save('translate_Audio.mp3')
        os.system("translate_Audio.mp3")
